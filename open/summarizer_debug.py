...
class SummaryModel(object):
    ...
    def run_train_step(self, sess, doc_batch, summary_batch, label_batch, docLens, summaryLens):
        ...
        #_,u, dec_in, _l, _c = sess.run([self.decoder_outputs_train, self.u, self._decoder_in_state, self.train_op, self.check_op], feed_dict = feed_dict)
        #dec_in = sess.run(self.u, feed_dict = feed_dict)
        #var_grad = sess.run(self.var_grad,feed_dict = feed_dict )

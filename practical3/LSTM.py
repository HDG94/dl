...
with tf.variable_scope('all', reuse = False):
    ...
    init_state_c = tf.Variable(cell.zero_state(BATCH_SIZE, dtype=tf.float32)[0], trainable = False)
    init_state_h = tf.Variable(cell.zero_state(BATCH_SIZE, dtype=tf.float32)[1], trainable = False)
    init_assign_c = init_state_c.assign(cell.zero_state(BATCH_SIZE, dtype=tf.float32)[0])
    init_assign_h = init_state_h.assign(cell.zero_state(BATCH_SIZE, dtype=tf.float32)[1])
    rnn_outputs, (init_state_c, init_state_h) = tf.nn.dynamic_rnn(cell, tf.nn.dropout(rnn_inputs, DROPOUT), sequence_length = seqlen, initial_state = tf.nn.rnn_cell.LSTMStateTuple(init_state_c, init_state_h), time_major = False) #TODO: remember to feed in the np_state and store back
...
with tf.variable_scope('all', reuse = True):
    ####################### Testing Graph
    # RNN LAYER:
    rnn_outputs_valid, (init_state_c, init_state_h) = tf.nn.dynamic_rnn(cell, rnn_inputs, sequence_length = seqlen, initial_state = tf.nn.rnn_cell.LSTMStateTuple(init_state_c, init_state_h), time_major = False)
...
outfile = open ('resultsLSTM_untrainableStates.txt', 'w')
for i in range(EPOCH):
    ...
    for tup in sf.get_batch(): # getting a batch of *documents*
        ...
        sess.run(init_assign_c)
        sess.run(init_assign_h)

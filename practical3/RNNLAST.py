...
with tf.variable_scope('all', reuse = False):
	...
	#TODO
    np_state = None
    init_state_placeholder = tf.placeholder(tf.float32, shape = cell.zero_state(BATCH_SIZE, dtype = tf.float32).get_shape())
    ...
    rnn_outputs, init_state = tf.nn.dynamic_rnn(cell, tf.nn.dropout(rnn_inputs, DROPOUT), sequence_length = seqlen, initial_state = init_state, time_major = False) #TODO: remember to feed in the np_state and store back
    # Mask
    ...
    x = last_relevant(rnn_outputs, seqlen) # reduce to dim(BS*DIM_WE)
...
with tf.variable_scope('all', reuse = True):
	...
	# x layer after summation
    x_valid = last_relevant(rnn_outputs_valid, seqlen) # reduce to dim(BS*DIM_WE)
	# HIDDEN LAYER:
        ...
        for tup_validation in sf_v.get_batch():
            ...
            for ns in range(num_sentences):
                ...
                for row in range(len(x_temp)):
                    if not (np.all(x_temp[row]==0)):
                        np_x_sums[row] = np.copy(x_temp[row])

            print ('the output state of test set:')
            print (np_x_sums)

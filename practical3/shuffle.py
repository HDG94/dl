    ...
    def get_batch(self):
        ...
        for i in range(self.get_iteration()):
            ...
            if (end < self.N):
                ...
                label = [tup[1] for tup in self.data_label[start:end]]
                lens = [tup[2] for tup in self.data_label[start:end]]
                yield [i, data, label, lens]
            else:
                # concatenate the two parts
                data = [tup[0] for tup in self.data_label[start:]]
                label = [tup[1] for tup in self.data_label[start:]]
                lens = [tup[2] for tup in self.data_label[start:]]
                N2 = end - self.N
                data += [tup[0] for tup in self.data_label[0: N2]]
                label += [tup[1] for tup in self.data_label[0: N2]]
                lens += [tup[2] for tup in self.data_label[0: N2]]
                yield [i, data, label, lens]
